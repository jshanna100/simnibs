...
orig_dir=$root_dir"/simnibs/4/"

for subj in "${subjs[@]}"
do
   echo "$subj"
   subj_dest="$dest_dir$subj"
   cp "$orig_dir""$subj"/m2m_"$subj"/charm_report.html "$orig_dir"reports/"$subj"_report.html
   cp "$orig_dir""$subj"/m2m_"$subj"/charm_log.html "$orig_dir"logs/"$subj"_log.html
done
